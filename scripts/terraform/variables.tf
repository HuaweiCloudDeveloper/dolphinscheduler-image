...
variable "instance_flavor_cpu" {
  description = " Set the cpu (The configuration must meet the offering configuration requirements and the parameter settings must comply with the general computing-plus specifications of the x86 architecture.)"
  nullable    = false
  type    = number
  default = 4
}
...
variable "instance_flavor_memory" {
  description = " Set the memory size (The configuration must meet the offering configuration requirements and the parameter settings must comply with the general computing-plus specifications of the x86 architecture.)"
  nullable    = false
  type    = number
  default = 8
}
...
variable "Version" {
  ...
  validation {
    condition     = contains(["DolphinScheduler3.2.2-x86-v1.0"], var.Version)
